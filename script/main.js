...
const screens = {
    ...
    orientation_error: {
        render() {
            ctx.textAlign = textAlign.horizontal.center;
            ctx.textBaseline = textAlign.vertical.center;
            ctx.font = `${fonts.title.size}px '${fonts.title.font}'`;
            ctx.fillStyle = colorSchema["branco"].normal;
            ctx.fillText("Change orientation to landscape", canvas.width / 2, canvas.height / 2);
        }
    }
}
